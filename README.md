...
Where2Go is being restarted as a World of Warcraft addon for deciding which
content is most worthwhile to run when the player has a set of desired items.

The repository intentionally contains only the product brief, implementation
plan, and recorded decisions. The previous implementation is preserved at the
`codex/pre-restart-backup` branch.
...
Read the following before restarting implementation:
