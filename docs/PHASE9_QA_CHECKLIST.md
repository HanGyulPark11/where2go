...
- [o] Window opens, roughly 860px wide, three visible list columns side
      by side with their own headers: **Results | Staged | Preferred**.
- [o] The old separate "Dungeon" and "Boss" button rows are **gone** —
      replaced by a single dropdown labeled "Source" area (button reads
      "All Sources" by default).
- [o] Window is draggable by its title bar / chrome (not by hovering an
      item row — see § 6 "Known issues" below, this is expected).

## 2. Source dropdown (multi-select)

- [o] Click the Source dropdown. It opens a list with a **"Dungeons"**
      group header, followed by every dungeon, then one group header per
      raid (the raid's own name), followed by that raid's bosses.
- [o] Each entry has a **checkbox**, not a radio dot.
- [o] Click one dungeon entry: the menu **stays open** (does not
...
- [o] Click a second entry (e.g. a raid boss) while the first is still
      checked: both stay checked, Results shows the **union** of both
      (not just the second one, not empty).
- [o] Close the dropdown. Its own button text now reads "2 selected" (or
      the correct count).
- [o] Uncheck both entries (reopen, click each again): Results returns to
      showing everything, button text returns to "All Sources".

## 3. Slot / stat / eligible / search filters (should behave as before)

- [o] Slot buttons show **friendly names** ("Head", "Trinket", "Main
      Hand", etc.), not raw ids like `HEAD`/`TRINKET`.
- [o] Clicking a slot button narrows Results to that slot; clicking it
      again clears the filter.
- [o] Stat checkboxes (Crit/Haste/Mastery/Versatility) narrow Results to
      items with **all** checked stats (not just any one).
- [o] The **spec dropdown now sits directly next to** the "Selected spec
      eligible only" checkbox (previously it was up near the Drop/
      Voidcore buttons — confirm it moved).
- [o] Checking "eligible only" narrows Results to items your currently
      selected spec can use.
- [o] There's a small label above/near the search box (new — previously
...
- [o] Check a checkbox on 2-3 Results rows. Each checked item
      **immediately appears** in the Staged column (icon/name/summary,
      same treatment as Results, no checkbox — instead a small `×` on the
      right).
- [o] Click the `×` on one Staged row: it disappears from Staged, **and**
      its checkbox back in Results unchecks itself.
- [o] Stage more than 10 items (check enough Results rows). **New in
      ...
      at 10.
- [o] Click "Add selected": staged items move into the Preferred column
      (see next section), and the Staged column empties.
- [o] Stage a few more items, click "Clear selection": Staged empties
      without adding anything to Preferred.

## 6. Preferred panel

- [o] After "Add selected" above, the added items appear in the
...
- [o] Click the `×` on one Preferred row: just that item is removed,
      nothing else changes.
- [o] Add more than 10 preferred items total. **New in this pass:**
...
- [o] Click "Clear All": a confirmation popup appears ("Remove every
      preferred item from the current list?"). Confirm it, and the whole
      Preferred list empties. Cancel it once too, to confirm cancel
      actually cancels.
- [o] Switch the Drop/Voidcore mode toggle (top of window): Preferred
...
- [o] Existing behavior unchanged: cards per dungeon/boss, click a card
      header to expand/collapse, Drop/Voidcore tabs switch views.
- [o] Expanded card item rows now use the **same icon + quality-colored
      name + ilvl/stat summary** treatment as the browser (previously
      plain text lines).
- [o] Hover an item row inside an expanded card: real tooltip appears
      ...
      a gap.
- [o] Cards resize correctly when expanded/collapsed with the new taller
...
- [o] Item Browser title, Drop/Voidcore labels, Source dropdown group/
      ...
      in Korean.
- [o] Slot filter buttons show Korean slot names.
- [o] The Recommendation panel's title, Drop/Voidcore tabs, and Browse
...
- [o] Item names and tooltips are in Korean too — but that's the WoW
      client doing it automatically, not this addon; if it's wrong, it's
      not a Phase 9 bug.
- [o] Dungeon/boss/raid names stay in **English** regardless of client
...
- [o] **Search box label may overlap the stat-checkbox row above it.**
...
- [o] **Results list is not sorted** — items display in raw
...
- [o] **Dragging the window by an item row doesn't work** — item rows now
      ...
      broken in practice.
- [o] **Cold item cache on first open** — if you open the browser or
